...
// (يرجى التأكد من أن هذه المسارات صحيحة)
...
  const navigate = useNavigate();
  ...
  // 👇 حالة مودال مسح السلة
  const [isClearingCart, setIsClearingCart] = useState(false); 
  // 👇 حالة لعرض أخطاء المخزون عند التحديث (بدلاً من alert)
  const [stockError, setStockError] = useState(null); 

  const handleQuantityChange = (itemId, newQuantity, maxStock) => {
    setStockError(null); // مسح الأخطاء عند محاولة التغيير

    if (newQuantity < 1) return;
    if (newQuantity > maxStock) {
      setStockError(`Only ${maxStock} units available in stock.`); // 👈 استبدال alert
      return;
    }
    updateQuantity?.(itemId, newQuantity);
  };

  // دالة تأكيد مسح السلة
  const handleConfirmClearCart = () => {
    clearCart?.();
    setIsClearingCart(false);
  };
  
  // دالة إزالة عنصر واحد (استبدال window.confirm)
  const handleRemoveItem = (itemId) => {
    // نستخدم حذف مباشر لعنصر واحد لتبسيط التجربة، بدلاً من مودال
    removeFromCart?.(itemId);
  };
  
  const hasStockIssue = cartItems.some(item => item.quantity > item.stock_quantity);
...
          </Link>
        </div>
        {/* تأكد من عدم عرض المودال في هذه الحالة */}
      </div>
    );
  }
...
        </div>
        ...
        {/* 👇 عرض أخطاء المخزون (من handleQuantityChange) */}
        {stockError && (
          <div className="mb-6">
...
                        <button
                          onClick={() =>
                            handleQuantityChange(item.id, item.quantity - 1, item.stock_quantity)
                          }
...
                      <button
                        onClick={() => { handleRemoveItem(item.id) }} // 👈 تم التعديل
...
              <Link to="/products" className="block w-full btn btn-outline text-center">
                Continue Shopping
              </Link>
